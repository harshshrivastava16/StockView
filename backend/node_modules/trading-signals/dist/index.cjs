...
var StochasticOscillator = class extends TechnicalIndicator {
  ...
  update(candle, replace) {
    ...
    if (this.candles.length === this.n) {
      ...
      const divisor = new import_big22.default(highest).minus(lowest);
      let fastK = new import_big22.default(100).mul(new import_big22.default(candle.close).minus(lowest));
  ...
  update(price, replace) {
    ...
    if (rsiResult) {
      ...
      if (periodResult) {
        ...
        if (denominator.eq(0)) {
          return this.setResult(new import_big23.default(100), false);
        }
...
var LinearRegression = class extends TechnicalIndicator {
  ...
  calculateRegression(prices) {
    const n = new import_big24.default(prices.length);
    const xValues = Array.from({ length: prices.length }, (_, i) => new import_big24.default(i));
    const yValues = prices.map((p) => new import_big24.default(p));
    const sumX = xValues.reduce((sum, x) => sum.add(x), new import_big24.default(0));
    const sumY = yValues.reduce((sum, y) => sum.add(y), new import_big24.default(0));
    const sumXY = xValues.reduce((sum, x, i) => sum.add(x.mul(yValues[i])), new import_big24.default(0));
    const sumXX = xValues.reduce((sum, x) => sum.add(x.mul(x)), new import_big24.default(0));
  ...
  update(price, replace) {
    ...
    if (this.prices.length === this.interval) {
      const weightedPricesSum = this.prices.reduce((acc, price2, index) => {
        const weightedPrice = new import_big25.default(price2).mul(index + 1);
        return acc.add(weightedPrice);
      }, new import_big25.default(0));
